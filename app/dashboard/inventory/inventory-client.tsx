...
export default function InventoryClientComponent({
  ...
  return (
    <>
      <Topbar title="Inventory" subtitle={`${inventory.length} items`} />
      <main className="flex-1 px-4 py-6 lg:px-7 lg:py-8">
        <PageHeader
          title="Inventory"
          description="All assets in your rental catalog. Click an item to see availability, history and bookings."
          actions={
            <>
        ...
        <div className="mt-6 rounded-2xl border border-border bg-surface">
          <div className="flex flex-wrap items-center gap-2 border-b border-border p-3">
            ...
            <FilterMenu label={cat ?? "Category"} active={!!cat}>
              <DropdownMenuItem onClick={() => setCat(null)}>All categories</DropdownMenuItem>
            ...
            <FilterMenu label={status ?? "Status"} active={!!status}>
              <DropdownMenuItem onClick={() => setStatus(null)}>Any status</DropdownMenuItem>
              <DropdownMenuSeparator />
            ...
            <table className="w-full min-w-[860px] text-sm">
              <thead>
                <tr className="text-left text-[11px] uppercase tracking-wider text-muted-foreground">
                  ...
                  <th className="px-3 py-2.5 font-semibold text-right">Price / day</th>
                  ...
                  <th className="px-3 py-2.5 font-semibold text-right">Rented</th>
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-muted/40">
                    ...
                    <td className="px-3 py-3">
                      <StatusPill tone={statusTone(item.status) as never}>{item.status}</StatusPill>
                    </td>
          ...
          <div className="flex items-center justify-between border-t border-border px-5 py-3 text-xs text-muted-foreground">
            <div>
              Showing <span className="font-medium text-foreground">{filtered.length}</span> of {inventory.length}
            </div>
